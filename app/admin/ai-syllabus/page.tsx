...
export default function AdminAISyllabus() {
  ...
  // Fetch AI Fundamentals syllabus data
...
const colorOptions = [
  ...
  const getColorStyle = (colorClass: string) => {
    ...
    return color ? color.preview : '#5433FF';
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading AI for Builders syllabus...</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md">
            <h3 className="text-lg font-semibold mb-4">Delete Week?</h3>
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete Week {weeks[deleteConfirm]?.weekNumber}: {weeks[deleteConfirm]?.weekName}?
              This action cannot be undone.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => {
                  deleteWeek(deleteConfirm);
                  setDeleteConfirm(null);
                }}
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">AI for Builders Syllabus</h1>
            <p className="mt-2 text-gray-600">Manage your AI for Builders course curriculum</p>
          </div>
          <button
            onClick={addNewWeek}
      ...
      {!isLoading && Object.keys(weeks).length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">No AI for Builders syllabus data found.</p>
          <a href="/seed-ai-syllabus" className="text-blue-600 hover:text-blue-700 font-medium">
        ...
        <>
                  ...
                  <div
                    ...
                    onDrop={(e) => {
                      ...
                      if (draggedWeek && draggedWeek !== week.id) {
                        const draggedIndex = Object.values(weeks).findIndex(w => w.id === draggedWeek);
                        const dropIndex = index;
                        moveWeek(draggedIndex, dropIndex);
                        setDraggedWeek(null);
                      }
                    }}
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {index > 0 && (
                          <button
                            onClick={() => moveWeek(index, index - 1)}
                        ...
                        {index < Object.values(weeks).length - 1 && (
                          <button
                            onClick={() => moveWeek(index, index + 1)}
                            className="text-gray-400 hover:text-gray-600 transition-colors"
                            title="Move down"
                ...
                {Object.values(weeks)
                  ...
                  .map((week, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {week.tools.slice(0, 3).map((tool, idx) => (
                            <span
                              key={idx}
                              className="inline-block px-2.5 py-1 rounded-full text-xs font-medium text-blue-700 bg-blue-100"
                            >
                              {tool}
                            </span>
                          ))}
                          {week.tools.length > 3 && (
                            <span className="inline-block px-2.5 py-1 rounded-full text-xs font-medium text-gray-500">
                              +{week.tools.length - 3} more
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div className="mt-8 flex justify-center gap-4">
            ...
            >
              ...
              View AI for Builders Page
            </a>
            <a
              href="/seed-ai-syllabus"
              className="text-blue-600 hover:text-blue-700 font-medium flex items-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
