...
interface AIFundamentalsClientProps {
...
export default function AIFundamentalsClient({ pageContent, syllabus, cohortDate, cohortDates }: AIFundamentalsClientProps) {
  ...
  const handleSectionClick = useCallback((sectionId: string) => {
    ...
    if (element) {
      const offset = 120; // Account for fixed navigation and some breathing room
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Use a position closer to the top of viewport for better section detection
      const scrollPosition = window.scrollY + 150; // Offset to account for fixed nav
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <motion.div 
        className="fixed top-0 left-0 right-0 h-1 bg-blue z-[60] origin-left"
        style={{ 
      ...
      >
        ...
        <div className="relative z-10 text-center text-white px-8 max-w-4xl mx-auto">
          ...
          <motion.button 
            className="px-12 py-6 bg-blue text-white font-semibold rounded-full text-lg hover:bg-purple-600 transition-all duration-300 shadow-xl"
            initial={{ opacity: 0, y: 30 }}
            animate={heroInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
          ...
          >
            {cohortDate.startDate || 'Start Learning'}
          </motion.button>
        </div>
        ...
        <div id="section-syllabus" className="bg-gray-50 pt-16 pb-32 md:pb-40">
          <div className="max-w-4xl mx-auto px-8">
            ...
            >
              ...
              <VerticalSyllabusCards serverSyllabus={syllabus} courseType="ai-fundamentals" />
            </motion.div>
          </div>
        </div>
