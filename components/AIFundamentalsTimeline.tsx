...
interface AIFundamentalsTimelineProps {
  serverSyllabus: SyllabusItem[];
}

export default function AIFundamentalsTimeline({ serverSyllabus }: AIFundamentalsTimelineProps) {
  ...
  const weeks: Week[] = serverSyllabus.map(week => ({
    ...
    color: week.color || '#5433FF',
    topics: Array.isArray(week.tools) ? week.tools : []
  }));
                ...
                style={{
                  boxShadow: selectedWeek === index 
                    ? (() => {
                        ...
                        const rgb = colorMap[week.color] || '84, 51, 255';
              ...
              {selectedWeek === index && (
                <motion.div
                  ...
                  style={{
                    ...
                    borderTop: `12px solid ${week.color === 'bg-blue' ? '#5433FF' : week.color === 'bg-purple-600' ? '#9333EA' : week.color === 'bg-mint' ? '#42EAFF' : week.color === 'bg-coral' ? '#FD5E53' : '#FFB343'}`,
                  }}
